...
def get_user_key(fid: int) -> str:
    return f"user:{fid}"


def get_user_notification_details_key(fid: int) -> str:
    return f"user:{fid}:notifications"


def create_or_update_user(fid: int, username: str = "") -> None:
    user_key = get_user_key(fid)
...
def send_notification(fid: int, project_name: str, text: str) -> dict:
    """Send a notification to a user about their frame project
    ...
    """
    print(f"Attempting to send notification to FID {fid} for project {project_name}")
    try:
